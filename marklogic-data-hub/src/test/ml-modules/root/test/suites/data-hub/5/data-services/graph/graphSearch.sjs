...
resultsTest4.nodes.forEach(node => {
  if(node.id === "http://marklogic.com/example/BabyRegistry-0.0.1/BabyRegistry/3039-42_/content/babyRegistry1.json") {
    assertions.push(test.assertFalse(node.hasRelationships), "BabyRegistry 3039-42 must have relationships flag in false.");
  }
  else if(node.id === "http://marklogic.com/example/BabyRegistry-0.0.1/BabyRegistry/3039-42__/content/babyRegistry1.json-Product") {
    assertions.push(test.assertFalse(node.hasRelationships), "Product group node must have relationships flag in false.");
  }
  else if(node.id === "http://example.org/Customer-0.0.1/Customer/301_/content/customer1.json") {
...
assertions.concat([
  ...
  test.assertEqual("http://example.org/Customer-0.0.1/Customer/301_/content/customer1.json", resultsTest6.edges[0]["from"]),
  test.assertEqual("http://marklogic.com/example/BabyRegistry-0.0.1/BabyRegistry/3039-42_/content/babyRegistry1.json", resultsTest6.edges[0]["to"], xdmp.toJsonString(resultsTest6.edges[0]))
]);
...
resultConceptWithHasRelationship.nodes.forEach(node => {
  if(node.id === "http://example.org/Office-0.0.1/Office/1_/content/office1.json") {
